...
const PopularGiftMaster = () => {
    ...
    const paginate = (clear = false, isNewFilter = false) => {
        ...
        setLoading(true);
        API.get(apiConfig.allPopularGift)
            .then((res) => {
                setLoading(false);
                setState({
                    data: res,
    ...
    const handleSaveButtonClick = () => {
        if (rowMoved) {
            ...
            API.put(apiConfig.allPopularGift, payload)
                .then((res) => {
                    toaster.success('Popular Gift updated successfully');
                    setRowMoved(false);
                    paginate();
                })
                .catch((error) => {
    ...
    const onClickDelete = (id) => {
        Swal.fire({
            title: "Are You Sure",
            text: "Are you sure you want to remove this Popular Gift ?",
            icon: "question",
            ...
            if (result.isConfirmed) {
                API.destroy(`${apiConfig.allPopularGift}/${id}`)
                    .then((res) => {
...
export default PopularGiftMaster;
