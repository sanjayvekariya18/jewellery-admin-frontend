...
const PopularEngagement = () => {
    ...
    const paginate = (clear = false, isNewFilter = false) => {
        ...
        setLoading(true);
        API.get(apiConfig.allPopularEngagement)
            .then((res) => {
                setLoading(false);
                setState({
                    data: res,
    ...
    const handleSaveButtonClick = () => {
        if (rowMoved) {
            ...
            API.put(apiConfig.allPopularEngagement, payload)
                .then((res) => {
                    toaster.success('Popular Engagement updated successfully');
                    setRowMoved(false);
                    paginate();
                })
                .catch((error) => {
    ...
    const onClickDelete = (id) => {
        Swal.fire({
            title: "Are You Sure",
            text: "Are you sure you want to remove this Popular Engagement ?",
            icon: "question",
            ...
            if (result.isConfirmed) {
                API.destroy(`${apiConfig.allPopularEngagement}/${id}`)
                    .then((res) => {
...
export default PopularEngagement;
